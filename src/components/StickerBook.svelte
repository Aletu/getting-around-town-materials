...
<script>
  ...
  $: badgeStickers = Object.entries(stickerDefinitions).filter(
    ([_, def]) => def.type === "badge",
  );

  $: counterStickers = Object.entries(stickerDefinitions).filter(
    ([_, def]) => def.type === "counter",
  );
  ...
  $: earnedBadges = badgeStickers.filter(
    ([key, _]) => $stickersStore[key],
  ).length;
...
{#if isOpen}
  ...
  >
    ...
    >
      <!-- Header -->
      <div
        class="bg-gradient-to-r from-primary/20 via-secondary/20 to-accent/20 p-6 border-b border-base-200"
      >
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-8">
        <!-- Daily Streak -->
        <div
          class="bg-gradient-to-r from-warning/10 to-accent/10 rounded-2xl p-4"
        >
          <div class="flex items-center justify-between">
            ...
            <div class="text-right">
              <div class="text-3xl font-black text-warning">
                {$stickersStore.dailyStreak || 0}
              </div>
              <div class="text-xs text-base-content/50">
                Best: {$stickersStore.bestStreak || 0}
              </div>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {#each counterStickers as [key, def]}
              <div
                class="bg-base-200/50 rounded-xl p-4 text-center hover:bg-base-200 transition-colors"
              >
                <div class="text-3xl mb-2">{def.emoji}</div>
                <div class="font-bold text-xl text-accent">
                  {$stickersStore[key] || 0}
                </div>
            ...
            {#each badgeStickers as [key, def]}
              ...
              <div
                class="relative group rounded-2xl p-4 text-center transition-all duration-300 border-2
                  ...
                  : 'bg-base-200/30 border-base-300 grayscale opacity-50'}"
              >
                <div
                  class="text-4xl mb-2 transition-transform duration-300 group-hover:scale-110"
                >
                  {def.emoji}
                </div>
      ...
      <!-- Footer -->
      <div class="p-4 border-t border-base-200 bg-base-100 flex gap-3">
        <button
        ...
        <button
          class="btn btn-secondary rounded-xl font-bold gap-2"
        ...
        >
          ...
          <span class="hidden sm:inline">Print Score</span>
        </button>
      </div>
    </div>
  </div>
