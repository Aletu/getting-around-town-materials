<script>
  ...
  function nextStep() {
    ...
    if (step === 4 && selectedTransport) {
      safetyTipIndex = Math.floor(
        Math.random() * selectedTransport.tips.length,
      );
    }
...
<div class="max-w-3xl mx-auto space-y-8">
  ...
  >
      ...
      <div in:fly={{ y: 12, duration: 250 }} class="space-y-5">
        <div class="text-center space-y-1">
          <div class="flex items-center justify-center gap-3">
            ...
            <SpeakButton
              text="What do you need to bring? Select the items to put in your bag."
              size="sm"
            />
          </div>
          <p class="text-sm text-base-content/60">
            Select the items to put in your bag.
          </p>
        </div>
    ...
    {#if step === 3}
      <div in:fly={{ y: 12, duration: 250 }} class="space-y-5">
        <div class="flex items-center justify-center gap-3">
          <h2 class="text-xl font-bold text-center text-base-content">
            How will you get there?
          </h2>
          <SpeakButton text="How will you get there?" size="sm" />
        </div>
    ...
    {#if step === 4}
      <div in:fly={{ y: 12, duration: 250 }} class="space-y-6 text-center">
        ...
        <div
          class="card bg-base-200/50 border border-base-200 max-w-sm mx-auto rounded-xl"
          id="printable-plan"
        >
          <div class="card-body text-left space-y-4 p-5">
            <!-- Header for Print only -->
            <div
              class="hidden print:block text-center border-b-2 border-primary pb-4 mb-4"
            >
            ...
            <!-- Destination -->
            <div
              class="flex items-center gap-3 border-b border-base-300/50 pb-4"
            >
              <div class="text-3xl bg-base-100 p-2.5 rounded-xl shadow-sm">
                {selectedPlace.emoji}
              </div>
              <div>
                <div
            ...
            <div>
              <div
                class="text-xs uppercase font-semibold text-base-content/50 mb-2"
              >
                In Your Bag
              </div>
  ...
  <div class="flex justify-between pt-2">
    ...
    {#if step < 4}
      <button
        class="btn btn-primary gap-2 px-6 rounded-xl font-semibold shadow-sm hover:shadow-md transition-all"
        on:click={nextStep}
      >
        Next
        <svg
