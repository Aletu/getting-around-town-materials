<script>
  ...
  $: if (
    Array.isArray($shortQAStore) &&
    $shortQAStore.length > 0 &&
    items.length === 0
  ) {
    startSession();
  }
...
{#if $teacherMode}
  <div
    class="bg-base-100 rounded-2xl shadow-soft p-6 lg:p-8 mb-8 border border-base-200/50"
  >
    <div
      class="flex items-center justify-between mb-6 pb-5 border-b border-base-200"
    >
      <div class="flex items-center gap-3">
        ...
        <div>
          <h2 class="text-xl lg:text-2xl font-bold text-base-content">
            Q&A Editor
          </h2>
          <p class="text-sm text-base-content/60">
            Manage the multiple choice questions.
          </p>
        </div>
      </div>
      <div class="badge badge-primary font-semibold">
        {$shortQAStore.length} Questions
      </div>
    </div>
    ...
    <div class="space-y-3">
      {#each $shortQAStore as item, i}
        <div
          class="collapse collapse-arrow bg-base-200/50 border border-base-200 rounded-xl hover:border-base-300 transition-colors"
        >
          <input type="checkbox" />
          <div
            class="collapse-title text-base font-semibold flex items-center gap-3 pr-12"
          >
            <span class="badge badge-neutral badge-sm font-mono">{i + 1}</span>
            <span class="truncate flex-1 text-base-content/90"
              >{item.question || "New Question"}</span
            >
          </div>
          <div
            class="collapse-content bg-base-100 pt-4 border-t border-base-200 rounded-b-xl"
          >
            <div class="grid gap-5 p-1">
              ...
              <div
                class="bg-base-200/30 p-4 rounded-xl border border-base-200/50"
              >
                <div
                  ...
                  {#each item.options as opt}
                    ...
                    >
                      <input
                        ...
                        checked={item.answer === opt.id}
                        on:change={() => (item.answer = opt.id)}
                      />
                      <span
                        class="font-mono font-bold text-sm text-base-content/50 w-5"
              ...
              <div class="flex justify-end pt-2">
                <button
                  class="btn btn-ghost btn-sm text-error hover:bg-error/10 gap-2 font-medium"
    ...
    <button
      class="btn btn-primary w-full mt-6 rounded-xl shadow-sm hover:shadow-md transition-all gap-2 font-semibold"
      on:click={addItem}
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
...
{:else}
  <section class="space-y-6 max-w-3xl mx-auto" in:fade={{ duration: 300 }}>
    ...
    <div class="flex items-center gap-4">
      <button
        class="btn btn-circle btn-ghost bg-base-100 shadow-sm hover:shadow-md hover:bg-base-200 transition-all"
        on:click={() => dispatch("back")}
        aria-label="Go back"
      >
        <svg
    ...
    {#if !finished && items.length > 0 && items[currentIndex]}
      <div
        class="card bg-base-100 shadow-soft overflow-hidden border border-base-200/50"
      >
        <!-- Progress Bar -->
        ...
        <div class="card-body p-5 sm:p-7 lg:p-8">
          ...
          <div class="flex items-center justify-between text-sm mb-5">
            <span class="text-base-content/50 font-medium"
              >Question {currentIndex + 1} of {items.length}</span
            >
            <div class="flex items-center gap-4">
          ...
          <!-- Question Card -->
          <div
            class="bg-gradient-to-br from-base-200/60 to-base-200/30 p-5 sm:p-6 rounded-2xl border border-base-300/50 mb-6 relative overflow-hidden"
          >
            <div
              class="absolute top-0 left-0 w-1 h-full bg-gradient-to-b from-primary to-accent rounded-full"
            ></div>
            <div class="flex gap-4 items-start pl-3">
              <div
                class="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center flex-shrink-0 border border-primary/20"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
              ...
              <div class="flex-1">
                <div class="flex items-start justify-between gap-2">
                  <p
                    class="text-lg sm:text-xl font-semibold leading-relaxed text-base-content"
                    aria-live="polite"
                  >
          ...
          <!-- Footer Stats -->
          <div
            class="mt-6 pt-4 border-t border-base-200 flex justify-between items-center text-sm text-base-content/50"
          >
            <span
              >Attempts: <span class="font-semibold text-base-content/70"
                >{attempts}</span
              ></span
            >
          </div>
    ...
    {:else if finished}
      ...
      >
        <div
          class="h-1.5 bg-gradient-to-r from-success via-primary to-accent"
        ></div>
        <div class="card-body items-center text-center p-6 sm:p-8 lg:p-10">
          <!-- Success Icon -->
          <div
            class="w-16 h-16 rounded-2xl bg-gradient-to-br from-success/20 to-success/5 flex items-center justify-center mb-4 border border-success/20 animate-celebrate"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
          ...
          <h3 class="text-2xl sm:text-3xl font-bold mb-2 text-base-content">
            Well Done!
          </h3>
          <p class="text-base-content/60 mb-4 max-w-sm">
          ...
          <div class="flex items-center justify-center gap-1 mb-6">
            <span
              class="text-4xl animate-star-burst"
          ...
          <div class="grid grid-cols-2 gap-3 w-full max-w-xs mb-6">
            <div
              class="bg-gradient-to-br from-success/10 to-success/5 p-4 rounded-xl border border-success/20"
            >
              <div
                class="text-xs uppercase font-semibold tracking-wider text-success/70 mb-1"
              >
                Correct
              </div>
            ...
            <div
              class="bg-gradient-to-br from-primary/10 to-primary/5 p-4 rounded-xl border border-primary/20"
            >
              <div
                class="text-xs uppercase font-semibold tracking-wider text-primary/70 mb-1"
              >
                Accuracy
              </div>
              <div class="text-2xl font-bold text-primary">
                {items.length && attempts > 0
                  ? Math.round((score / attempts) * 100)
                  : 0}<span class="text-sm font-normal">%</span>
              </div>
            </div>
          </div>
          ...
          <div class="flex flex-col sm:flex-row gap-2.5 w-full max-w-xs">
            <button
              class="btn btn-primary flex-1 gap-2 rounded-xl font-semibold"
              on:click={restart}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
            ...
            <button
              class="btn btn-ghost flex-1 gap-2 rounded-xl font-medium"
              on:click={() => dispatch("back")}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
        ...
        <div class="card-body items-center text-center p-8">
          ...
          {#if Array.isArray($shortQAStore) && $shortQAStore.length === 0}
            <p class="text-warning mt-2 text-sm">
              No questions found. Please add some in Teacher Mode.
            </p>
          {/if}
        </div>
      </div>
